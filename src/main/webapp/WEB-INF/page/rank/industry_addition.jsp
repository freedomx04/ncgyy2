...
<script class="template IndustryAddition" type="text/x-jquery-tmpl">
	<div class="admin-content">
    	<div class="am-cf am-padding">
      		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">工业增加值</strong></div>
    	</div>
    	...
    	<div class="am-g">
			<div class="am-u-sm-12" style="height: 50px;">
				...
				<div class="padding-right-o am-fr" style="margin-left: 20px;">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							...
							<form id="importAddition-file-form" class="am-fl">
								<div class="am-form-file am-inline-block am-fr">
									<button type="button" class="am-btn am-btn-default am-btn-sm"> <i class="am-icon-cloud-upload"></i> 导入Excel</button>
			  						<input type="file" id="importAddition-file-input" name="uploadfile" accept=".xls, .xlsx, .xlsm">
								</div>
							</form>
			   			</div>
	  				</div>
				...
				<div class="padding-right-o am-fr">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							{{? it.editable}}
			     			<button type="button" class="am-btn am-btn-default industryAddition-update-btn"><span class="am-icon-save"></span> 编辑</button>
			     			<button type="button" class="am-btn am-btn-default industryAddition-delete-btn"><span class="am-icon-trash-o"></span> 删除</button>
			   				{{?}}
							<button type="button" class="am-btn am-btn-default industryAddition-export-btn"><span class="am-icon-save"></span> 导出Excel</button>
			   			</div>
	  				</div>
				</div>
			</div>
			...
			<div class="am-u-sm-12">
				...
				<table class="am-table am-table-striped am-table-hover table-main" id="industryAddition-table">
				</table>
	    	</div>
		</div>
	</div>
</script>
<script class="template IndustryAdditionDialog" type="text/x-jquery-tmpl">
	<div class="am-modal-dialog">
		<div class="am-modal-hd">上报信息
			<a class="am-close am-close-spin" data-am-modal-cancel href="javascript: void(0)">×</a>
		</div>
		<div class="am-modal-bd">
			<form class="am-form am-form-horizontal" id="industryAddition-input-form">
...
<script class="template IndustryAdditionList" type="text/x-jquery-tmpl">
<thead>
	<tr>
...
<tbody>
	{{~ it.data: industryAddition:index }}
		<tr class="table-tr">
	...
	Klass.prototype = Object.create(Object.prototype, {
		...
		init: {
			value: function($tar, editable) {
				...
				.on("click", ".industryAddition-update-btn", function() {
					...
					$.ajax({
						url: "industryAddition/detail",
						type: "POST",
						data:{ additionId: ids.toString() },
								...
								success: function(county){
									...
									$dialog.modal({
										...
										onConfirm: function(e) {
											var $modal = this;
											var $industryAdditionInputForm = $("#industryAddition-input-form");
											var monthly = $industryAdditionInputForm.find("#monthly").val();
											if ($industryAdditionInputForm.valid()) {//如果表单验证成功，则进行提交。  
												$.ajax({
													url: "industryAddition/edit",
													type: "POST",
													data:{
														monthly: monthly,
				...
				.on("click", ".industryAddition-delete-btn", function() {
					var active_td = $tar.find("tr.am-active").find("td[id]");
					var ids = new Array();
					$.each(active_td, function(el, vl) {
						ids.push($(vl).attr("id"));
					...
					$dialog.modal({
						...
						onConfirm: function(e) {
							var $modal = this;
							$.ajax({
								url: "industryAddition/delete",
								type: "POST",
								data: { additionIds: ids.toString() },
								success: function(data) {
									if (data.status == 0) {
										self.load();
										$modal.close($dialog.remove());
				...
				.on("change", "#importAddition-file-input", function() {
					$.ajax({
						url: "rankingReport/importReport?type=1",
						type: "POST",
						data: new FormData($("#importAddition-file-form")[0]),
						...
						success: function(data) {
							if(data.status != 0) {
								...
								$tar.find('#importAddition-file-input').val("");
							}
						},
						error: function(data) {
						}
				...
				.on("click", ".industryAddition-export-btn", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/export?monthly=" + monthly + "&type=1";
				})
				.on("click", ".template-download", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/template?monthly=" + monthly + "&type=1";
				})
				.on("change", "#industryAddition-table .box_all", function(e) {
					e.stopPropagation();
					$tar.find(".box").trigger("click");
					$tar.find(".box").prop("checked", this.checked);
				})
				.on("click", "#industryAddition-table .box", function(e) {
					e.stopPropagation();
					var $this = $(this);
		...
		getValidate: {
			value: function($dialog) {
				var self = this;
				var validatorr = $dialog.find("#industryAddition-input-form").validate({
				      errorElement: "em",
