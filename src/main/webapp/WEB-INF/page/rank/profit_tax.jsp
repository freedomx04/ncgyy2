	...
	<div class="admin-content">
    	...
    	<div class="am-g">
			<div class="am-u-sm-12" style="height: 50px;">
				...
				<div class="padding-right-o am-fr" style="margin-left: 20px;">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							...
							<form id="importTax-file-form" class="am-fl">
								<div class="am-form-file am-inline-block am-fr">
									<button type="button" class="am-btn am-btn-default am-btn-sm"> <i class="am-icon-cloud-upload"></i> 导入Excel</button>
			  						<input type="file" id="importTax-file-input" name="uploadfile" accept=".xls, .xlsx, .xlsm">
								</div>
							</form>
			   			</div>
	  				</div>
				...
				<div class="padding-right-o am-fr">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							{{? it.editable}}
			     			<button type="button" class="am-btn am-btn-default profitTax-update-btn"><span class="am-icon-save"></span> 编辑</button>
			     			<button type="button" class="am-btn am-btn-default profitTax-delete-btn"><span class="am-icon-trash-o"></span> 删除</button>
			   				{{?}}
							<button type="button" class="am-btn am-btn-default profitTax-export-btn"><span class="am-icon-save"></span> 导出Excel</button>
			   			</div>
	  				</div>
				</div>
			</div>
			...
			<div class="am-u-sm-12">
				...
				<table class="am-table am-table-striped am-table-hover table-main" id="profitTax-table">
				</table>
	    	</div>
		</div>
	</div>
</script>
<script class="template ProfitTaxDialog" type="text/x-jquery-tmpl">
	<div class="am-modal-dialog">
		<div class="am-modal-hd">上报信息
			<a class="am-close am-close-spin" data-am-modal-cancel href="javascript: void(0)">×</a>
		</div>
		<div class="am-modal-bd">
			<form class="am-form am-form-horizontal" id="profitTax-input-form">
...
<script class="template ProfitTaxList" type="text/x-jquery-tmpl">
<thead>
	<tr>
	...
	Klass.prototype = Object.create(Object.prototype, {
		...
		init: {
			value: function($tar, editable) {
				...
				.on("click", ".profitTax-update-btn", function() {
					...
					$.ajax({
						url: "profitTax/detail",
						type: "POST",
						data:{ taxId: ids.toString() },
								...
								success: function(county){
									...
									$dialog.modal({
										...
										onConfirm: function(e) {
											var $modal = this;
											var $profitTaxInputForm = $("#profitTax-input-form");
											var monthly = $profitTaxInputForm.find("#monthly").val();
											if ($profitTaxInputForm.valid()) {//如果表单验证成功，则进行提交。  
												$.ajax({
													url: "profitTax/edit",
													type: "POST",
													data:{
														monthly: monthly,
														countyId: $profitTaxInputForm.find("#county").val(),
				...
				.on("click", ".profitTax-delete-btn", function() {
					var active_td = $tar.find("tr.am-active").find("td[id]");
					var ids = new Array();
					$.each(active_td, function(el, vl) {
						ids.push($(vl).attr("id"));
					...
					$dialog.modal({
						...
						onConfirm: function(e) {
							var $modal = this;
							$.ajax({
								url: "profitTax/delete",
								type: "POST",
								data: { taxIds: ids.toString() },
								success: function(data) {
									if (data.status == 0) {
										self.load();
										$modal.close($dialog.remove());
				...
				.on("change", "#importTax-file-input", function() {
					$.ajax({
						url: "rankingReport/importReport?type=3",
						type: "POST",
						data: new FormData($("#importTax-file-form")[0]),
						...
						success: function(data) {
							if(data.status != 0) {
								...
								$tar.find('#importTax-file-input').val("");
							}
						},
						error: function(data) {
						}
				...
				.on("click", ".profitTax-export-btn", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/export?monthly=" + monthly + "&type=3";
				})
				.on("click", ".template-download", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/template?monthly=" + monthly + "&type=3";
				})
				.on("change", "#profitTax-table .box_all", function(e) {
					e.stopPropagation();
					$tar.find(".box").trigger("click");
					$tar.find(".box").prop("checked", this.checked);
				})
				.on("click", "#profitTax-table .box", function(e) {
					e.stopPropagation();
					var $this = $(this);
		...
		getValidate: {
			value: function($dialog) {
				var self = this;
				var validatorr = $dialog.find("#profitTax-input-form").validate({
				      errorElement: "em",
