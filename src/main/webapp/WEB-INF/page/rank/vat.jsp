	...
	<div class="admin-content">
    	...
    	<div class="am-g">
			<div class="am-u-sm-12" style="height: 50px;">
				...
				<div class="padding-right-o am-fr" style="margin-left: 20px;">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							...
							<form id="importVat-file-form" class="am-fl">
								<div class="am-form-file am-inline-block am-fr">
									<button type="button" class="am-btn am-btn-default am-btn-sm"> <i class="am-icon-cloud-upload"></i> 导入Excel</button>
			  						<input type="file" id="importVat-file-input" name="uploadfile" accept=".xls, .xlsx, .xlsm">
								</div>
							</form>
			   			</div>
	  				</div>
				...
				<div class="padding-right-o am-fr">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							{{? it.editable}}
			     			<button type="button" class="am-btn am-btn-default vat-update-btn"><span class="am-icon-save"></span> 编辑</button>
			     			<button type="button" class="am-btn am-btn-default vat-delete-btn"><span class="am-icon-trash-o"></span> 删除</button>
			   				{{?}}
							<button type="button" class="am-btn am-btn-default vat-export-btn"><span class="am-icon-save"></span> 导出Excel</button>
			   			</div>
	  				</div>
				</div>
			</div>
			...
			<div class="am-u-sm-12">
				...
				<table class="am-table am-table-striped am-table-hover table-main" id="vat-table">
				</table>
	    	</div>
		</div>
	</div>
</script>
<script class="template VatDialog" type="text/x-jquery-tmpl">
	<div class="am-modal-dialog">
		<div class="am-modal-hd">上报信息
			<a class="am-close am-close-spin" data-am-modal-cancel href="javascript: void(0)">×</a>
		</div>
		<div class="am-modal-bd">
			<form class="am-form am-form-horizontal" id="vat-input-form">
...
<script class="template VatList" type="text/x-jquery-tmpl">
<thead>
	<tr>
...
<tbody>
	{{~ it.data: vat:index }}
		<tr class="table-tr">
			...
			<td id="{{=vat.id}}">{{=index+1}}</td>
			<td class="ellipsis">{{=(vat.county != null ? vat.county.name : "" )}}</td>
			<td>{{=vat.total != null ? vat.total : ""}}</td>
			<td>{{=vat.yearGrowth != null ? vat.yearGrowth.toFixed(2) : ""}}</td>
		</tr>
	{{~}}
</tbody>
</script>
	...
	Klass.prototype = Object.create(Object.prototype, {
		...
		init: {
			value: function($tar, editable) {
				...
				.on("click", ".vat-update-btn", function() {
								...
								success: function(county){
									...
									$dialog.modal({
										...
										onConfirm: function(e) {
											var $modal = this;
											var $vatInputForm = $("#vat-input-form");
											var monthly = $vatInputForm.find("#monthly").val();
											if ($vatInputForm.valid()) {//如果表单验证成功，则进行提交。  
												$.ajax({
													url: "vat/edit",
													type: "POST",
													data:{
														monthly: monthly,
														countyId: $vatInputForm.find("#county").val(),
														vatId: $vatInputForm.find("#id").val(),
														total: $vatInputForm.find("#total").val(),
														yearGrowth: $vatInputForm.find("#yearGrowth").val(),
													},
													success: function(data) {
				...
				.on("click", ".vat-delete-btn", function() {
					var active_td = $tar.find("tr.am-active").find("td[id]");
					var ids = new Array();
					$.each(active_td, function(el, vl) {
						ids.push($(vl).attr("id"));
				...
				.on("change", "#importVat-file-input", function() {
					$.ajax({
						url: "rankingReport/importReport?type=6",
						type: "POST",
						data: new FormData($("#importVat-file-form")[0]),
						...
						success: function(data) {
							if(data.status != 0) {
								...
								$tar.find('#importVat-file-input').val("");
							}
						},
						error: function(data) {
						}
				...
				.on("click", ".vat-export-btn", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/export?monthly=" + monthly + "&type=6";
				})
				.on("click", ".template-download", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/template?monthly=" + monthly + "&type=6";
				})
				.on("change", "#vat-table .box_all", function(e) {
					e.stopPropagation();
					$tar.find(".box").trigger("click");
					$tar.find(".box").prop("checked", this.checked);
				})
				.on("click", "#vat-table .box", function(e) {
					e.stopPropagation();
					var $this = $(this);
		...
		getValidate: {
			value: function($dialog) {
				var self = this;
				var validatorr = $dialog.find("#vat-input-form").validate({
				      errorElement: "em",
