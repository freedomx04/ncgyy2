...
<script class="template IndustryOutput" type="text/x-jquery-tmpl">
	<div class="admin-content">
    	<div class="am-cf am-padding">
      		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">工业总产值</strong></div>
    	</div>
    	...
    	<div class="am-g">
			<div class="am-u-sm-12" style="height: 50px;">
				...
				<div class="padding-right-o am-fr" style="margin-left: 20px;">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							...
							<form id="importOutput-file-form" class="am-fl">
								<div class="am-form-file am-inline-block am-fr">
									<button type="button" class="am-btn am-btn-default am-btn-sm"> <i class="am-icon-cloud-upload"></i> 导入Excel</button>
			  						<input type="file" id="importOutput-file-input" name="uploadfile" accept=".xls, .xlsx, .xlsm">
								</div>
							</form>
			   			</div>
	  				</div>
				...
				<div class="padding-right-o am-fr">
					<div class="am-btn-toolbar am-fl">
			   			<div class="am-btn-group am-btn-group-sm">
							{{? it.editable}}
			     			<button type="button" class="am-btn am-btn-default industryOutput-update-btn"><span class="am-icon-save"></span> 编辑</button>
			     			<button type="button" class="am-btn am-btn-default industryOutput-delete-btn"><span class="am-icon-trash-o"></span> 删除</button>
			   				{{?}}
							<button type="button" class="am-btn am-btn-default industryOutput-export-btn"><span class="am-icon-save"></span> 导出Excel</button>
			   			</div>
	  				</div>
				</div>
			</div>
			...
			<div class="am-u-sm-12">
				...
				<table class="am-table am-table-striped am-table-hover table-main" id="industryOutput-table">
				</table>
	    	</div>
		</div>
	</div>
</script>
<script class="template IndustryOutputDialog" type="text/x-jquery-tmpl">
	<div class="am-modal-dialog">
		<div class="am-modal-hd">上报信息
			<a class="am-close am-close-spin" data-am-modal-cancel href="javascript: void(0)">×</a>
		</div>
		<div class="am-modal-bd">
			<form class="am-form am-form-horizontal" id="industryOutput-input-form">
				...
				<div class="am-form-group">
					<label for="enterpriseNum" class="am-u-sm-3 am-form-label font-normal">企业户数:</label>
		    		<div class="am-u-sm-9 padding-left-o">
						<input type="text" name="enterpriseNum" id="enterpriseNum" class="am-form-field"/>
		    		</div>
				</div>
...
<script class="template IndustryOutputList" type="text/x-jquery-tmpl">
<thead>
	<tr>
...
<tbody>
	{{~ it.data: industryOutput:index }}
		<tr class="table-tr">
			{{?it.editable}}<td><input type='checkbox' class='box' name='box'/></td>{{?}}
			<td id="{{=industryOutput.id}}">{{=index+1}}</td>
			<td class="ellipsis">{{=(industryOutput.county != null ? industryOutput.county.name : "" )}}</td>
	...
	Klass.prototype = Object.create(Object.prototype, {
		...
		init: {
			value: function($tar, editable) {
				...
				.on("click", ".industryOutput-update-btn", function() {
					...
					$.ajax({
						url: "industryOutput/detail",
						type: "POST",
						data:{ outputId: ids.toString() },
								...
								success: function(county){
									...
									$dialog.modal({
										...
										onConfirm: function(e) {
											var $modal = this;
											var $industryOutputInputForm = $("#industryOutput-input-form");
											var monthly = $industryOutputInputForm.find("#monthly").val();
											if ($industryOutputInputForm.valid()) {//如果表单验证成功，则进行提交。  
												$.ajax({
													url: "industryOutput/edit",
													type: "POST",
													data:{
														monthly: monthly,
														countyId: $industryOutputInputForm.find("#county").val(),
														outputId: $industryOutputInputForm.find("#id").val(),
				...
				.on("click", ".industryOutput-delete-btn", function() {
					var active_td = $tar.find("tr.am-active").find("td[id]");
					var ids = new Array();
					$.each(active_td, function(el, vl) {
						ids.push($(vl).attr("id"));
					...
					$dialog.modal({
						...
						onConfirm: function(e) {
							var $modal = this;
							$.ajax({
								url: "industryOutput/delete",
								type: "POST",
								data: { outputIds: ids.toString() },
								success: function(data) {
									if (data.status == 0) {
										self.load();
										$modal.close($dialog.remove());
				...
				.on("change", "#importOutput-file-input", function() {
					$.ajax({
						url: "rankingReport/importReport?type=5",
						type: "POST",
						data: new FormData($("#importOutput-file-form")[0]),
						...
						success: function(data) {
							if(data.status != 0) {
								...
								$tar.find('#importOutput-file-input').val("");
							}
						},
						error: function(data) {
						}
				...
				.on("click", ".industryOutput-export-btn", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/export?monthly=" + monthly + "&type=5";
				})
				.on("click", ".template-download", function() {
					var monthly = $tar.find(".monthly").val();
					location.href = "rankingReport/template?monthly=" + monthly + "&type=5";
				})
				.on("change", "#industryOutput-table .box_all", function(e) {
					e.stopPropagation();
					$tar.find(".box").trigger("click");
					$tar.find(".box").prop("checked", this.checked);
				})
				.on("click", "#industryOutput-table .box", function(e) {
					e.stopPropagation();
					var $this = $(this);
		...
		getValidate: {
			value: function($dialog) {
				var self = this;
				var validatorr = $dialog.find("#industryOutput-input-form").validate({
				      errorElement: "em",
