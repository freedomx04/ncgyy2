...
<body class="wx-addition">
			...
			<div class="weui_panel">
				<div class="weui_panel_bd">
					<div class="weui_media_box weui_media_text">
						<h4 class="weui_media_title">${addition.county.name}</h4>
						<table class="weui-table weui-border-tb" style="font-size: 14px; color: #999;">
							...
							<tbody>
								<tr>
									<td><fmt:formatNumber type="number" value="${addition.total}" pattern="0" maxFractionDigits="0"/></td>
									<td><fmt:formatNumber type="number" value="${addition.yearGrowth}" pattern="0.00" maxFractionDigits="2"/></td>
									<td>${addition.sort}</td>
								</tr>
							</tbody>
						</table> 
	...
	$(function( $ ) {
		...
		$('#search').on('click', function() {
			...
			$.ajax({
				url: '${ctx}/industryAddition/listByMonthly',
				type: 'POST',
				data: {
					monthly: monthly
				},
				success: function(ret) {
					if (ret.status == 0) {
						...
						$.each(ret.data, function(key, addition) {
							result += '<div class="weui_panel">'
								...
								+ '<td>' + (addition.yearGrowth != null ? addition.yearGrowth.toFixed(2) : "") + '</td>'
								+ '<td>' + (addition.sort || "") + '</td>'
