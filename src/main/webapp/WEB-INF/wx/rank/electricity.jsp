...
<body>
	...
	<div class="electricity-list">
		<c:forEach var="electricity" items="${electricityList}">
			<div class="weui_panel">
				<div class="weui_panel_bd">
					<div class="weui_media_box weui_media_text">
						<h4 class="weui_media_title">${electricity.county.name}</h4>
						<table class="weui-table weui-border-tb" style="font-size: 14px; color: #999;">
							...
							<tbody>
								<tr>
									<td><fmt:formatNumber type="number" value="${electricity.total}" pattern="0" maxFractionDigits="0"/></td>
									<td><fmt:formatNumber type="number" value="${electricity.yearGrowth}" pattern="0.00" maxFractionDigits="2"/></td>
	...
	$(function( $ ) {
		...
		$('#search').on('click', function() {
			...
			$.ajax({
				url: '${ctx}/electricity/listByMonthly',
				type: 'POST',
				data: {
					monthly: monthly
				},
				success: function(ret) {
					if (ret.status == 0) {
						...
						$.each(ret.data, function(key, electricity) {
							result += '<div class="weui_panel">'
								+ '<div class="weui_panel_bd">'
								+ '<div class="weui_media_box weui_media_text">'
								+ '<h4 class="weui_media_title">' + electricity.county.name + '</h4>'
								+ '<table class="weui-table weui-border-tb" style="font-size: 14px; color: #999;">'
								+ '<thead><tr><th>本月止累计(万千瓦时)</th><th>同比±%</th><th>增幅排序</th></tr></thead>'
								+ '<tbody><tr>'
